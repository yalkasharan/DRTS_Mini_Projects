...
def edf_processor_demand_test(tasks) -> tuple:
    ...
    # PDC requires evaluating all deadline checkpoints up to the hyperperiod.
    # Apply a safety cap (10^8) matching the simulation engine's MAX_HYPERPERIOD
    # to bound worst-case runtime on large co-prime period sets while still
    # being orders of magnitude larger than the old 10·max(T) heuristic.
    _PDC_CAP = 10 ** 8
    MAX_L = min(H, _PDC_CAP)
