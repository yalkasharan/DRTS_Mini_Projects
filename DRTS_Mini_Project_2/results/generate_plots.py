...
def chart3() -> bool:
    ...
    ax.set_xlabel("Streams   (label color: red = AVB-A, blue = AVB-B, grey = BE)", fontsize=11)
    ax.set_title("CBS vs Strict Priority WCD — Test Case 1")
    legend_items = [
        Patch(facecolor=CBS_GREEN, label="CBS WCD"),
        Patch(facecolor=SP_ORANGE, label="SP WCD"),
        Line2D([0], [0], color=DEADLINE_GRAY, linestyle="--", linewidth=1.5, label="Deadline"),
    ]
    ax.legend(handles=legend_items, loc="upper left")
    # Position subtitle at bottom of figure to avoid header overlap
    fig.text(0.5, 0.01, "CBS prevents starvation of lower-priority queues", ha="center", fontsize=11)
...
def main() -> None:
    configure_style()
    chart_functions: List[Callable[[], bool]] = [chart1, chart2, chart3, chart4, chart5, chart6, chart7, chart8]
    generated = sum(1 for chart_fn in chart_functions if chart_fn())
    print(f"Generated {generated}/8 charts successfully. Saved to results/plots/")
